...
class Init_basic(BaseModel):
    ...
    cold_source: Union[
        str, None] = None  # 冷源  chiller(冷水机组),integrated_evaporative_chiller（蒸发冷一体机),air_cooled_heat_pump（空气源热泵）
    heat_source: Union[
        str, None] = None  # 热源  boiler（锅炉）,integrated_evaporative_chiller（蒸发冷一体机),	air_cooled_heat_pump（空气源热泵）
    ...
    cold_water_model: Union[
        str, None] = None  # 冷水系统型式 冷水系统型式,first_pump_system_chiller（一次泵系统）,second_pump_system_chiller（二次泵系统）
    ...
    optimize_calculation_model: Union[str, None] = None  # 优化计算控制方式，当为"optimize_calculation_model"时计算控制方式: 不变水流量，否则是不变T1,T2,T3,T4，而变化水流量
    cooling_tower_t3: Union[str, None] = None  # 当为"cooling_tower_t3"时冷却塔出水温度是否设定值，如果是，怎T3按照百分比设定好的值计算，否则T3按照=ts+冷幅计算方式
...
class Load_rate_with_t_c(BaseModel):
    ...
    cool_out_t: Union[float, None] = None  # 冷却侧机组进水温度 T3
...
class Init_cooling_tower_free_calculation(BaseModel):  # 冷却塔免费冷源
    ...
    cooling_tower_free_sum: Union[int, None] = None  # 利用免费冷源连续运行的小时数
